...
export class Grass_Shader_Shadow extends Shader {
    ...
    update_GPU(context, gpu_addresses, graphics_state, model_transform, material) {
        ...
        context.uniform1f(gpu_addresses.light_depth_bias, 0.001);
                ...
                vec3 phong_model_lights( vec3 N, vec3 vertex_worldspace, 
                    ...
                    for(int i = 0; i < N_LIGHTS; i++){
                        ...
                        if (layer > 0.0){
                            ...
                            light_contribution = ground_color.xyz * light_colors[i].xyz * diffusivity * diffuse
                                                                  + light_colors[i].xyz * specularity * specular;
                            light_diffuse_contribution += attenuation * ground_color.xyz * light_colors[i].xyz * diffusivity * diffuse;
                            light_specular_contribution += attenuation * ground_color.xyz * specularity * specular;
                        }
                        result += attenuation * light_contribution;
                      }
                    return result;
    ...
    fragment_glsl_code() {
        return this.shared_glsl_code() + `
                ...
                void main(){
                    ...
                    if (layer > 0.0){
                        vec4 noise1 = texture2D(noiseTexture1, f_tex_coord / 1.3);
                        vec4 noise2 = texture2D(noiseTexture2, f_tex_coord * 2.0);
                        float alpha = ((noise1.x * 2.3) * pow((noise2.x * 3.0), 3.5)) - ((layer / 2.0 + 5.5) * 1.4 / 1.0);
                        if (alpha < 0.0 || worldPos.y < -0.7){
                            discard;
                        }
                        vec4 tex_color = texture2D(texture, f_tex_coord);
    ...
    vertex_glsl_code() {
        return this.shared_glsl_code() + `
                ...
                void main(){
                    ...
                    gl_Position = projection_camera_model_transform * vec4(position.x + (0.02 * alpha), position.y + (0.03 * layer), position.z + (0.02 * alpha), 1.0);
                    N = normalize( mat3( model_transform ) * normal / squared_scale);
                    vertex_worldspace = (model_transform * vec4( position, 1.0 )).xyz;
                }`;
    }
        ...
        return this.shared_glsl_code() + `
                ...
                void main(){
                    if (fake_shadow_layer){
                        ...
                        gl_FragColor = vec4(grass_color.x * ambient * 2.4 + (layer / 200.0), grass_color.y * ambient * 2.4 + (layer / 200.0), grass_color.z * ambient * 2.4 + (layer / 200.0), 1.0 - exp(-0.5 * (40.0 - distance(vec4(0,0,0,0), worldPos))));
                    }
                    else{
                    if (layer > 0.0){
...
//gl_FragColor = vec4(mix(shape_color.xyz, vec3(0.0,0.0,0.7), depthDifference),1.0);